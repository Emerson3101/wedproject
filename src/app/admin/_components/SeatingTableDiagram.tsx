...
function partyColor(partyKey: string): string {
        ...
        {slots.map((slot) => {
          ...
          if (!slot.occupied) {
            ...
            return (
              <div
                ...
                className="absolute w-9 h-9 -translate-x-1/2 -translate-y-1/2 rounded-full border border-dashed border-champagne/40 bg-transparent"
                style={{
                  left: `${slot.cx}%`,
                  top: `${slot.cy}%`,
                }}
          ...
          if (moveActive && !isMovingThis) {
            return (
              <div
                ...
                className={cn(
                  "absolute w-9 h-9 -translate-x-1/2 -translate-y-1/2 rounded-full border flex items-center justify-center text-xs font-display opacity-40",
                  table.shape === "round" && isTooCrowded && "w-7 h-7 text-[0.6rem]",
                  seat.is_lead
                    ? "bg-burgundy/15 border-burgundy/70 text-burgundy font-semibold"
                    : cn("bg-wine-deep/35", partyColor(seat.party_key))
                )}
                aria-hidden
              >
          ...
          return (
            <button
              ...
              className={cn(
                "absolute w-9 h-9 -translate-x-1/2 -translate-y-1/2 rounded-full border flex items-center justify-center text-xs font-display transition-all",
                table.shape === "round" && isTooCrowded && "w-7 h-7 text-[0.6rem]",
                seat.is_lead
                  ? "bg-burgundy/15 border-burgundy/70 text-burgundy font-semibold"
                  : cn("bg-wine-deep/35", partyColor(seat.party_key)),
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-1 max-h-44 overflow-auto pr-1 [scrollbar-width:thin]">
            {seatsOrdered.map((s) => (
              ...
              >
                <span
                  className={cn(
                    "inline-flex items-center justify-center w-6 h-6 rounded-full text-[0.625rem] font-display flex-shrink-0 border tabular-nums",
                    s.is_lead
                      ? "bg-burgundy/15 border-burgundy/70 text-burgundy font-semibold"
                      : cn("bg-wine-deep/35 border-champagne/30", partyColor(s.party_key))
                  )}
                  aria-hidden
                >
                  {s.seat_index + 1}
                ...
                <span
                  className={cn(
                    "truncate",
                    s.is_lead ? "text-burgundy font-medium" : "text-burgundy/80"
                  )}
                  title={s.seat_label}
                >
                  {s.seat_label}
            ...
            {parties.map((p) => {
              ...
              return (
                <li
                  key={p.partyKey}
                  className={cn(
                    "inline-flex items-center gap-2 px-2.5 py-1.5 rounded-full border text-xs",
                    partyColor(p.partyKey),
                    partyPending && "opacity-60"
                  )}
                >
                  <Users className="w-3.5 h-3.5" aria-hidden />
                  <span className="truncate max-w-[8rem] font-medium">{p.lead?.seat_label ?? "?"}</span>
                  {p.seats.length > 1 && (
                    <span className="text-burgundy/60 tabular-nums">+{p.seats.length - 1}</span>
                  )}
                  ...
                  <button
                    ...
                    title="Quitar grupo"
                    className="text-burgundy/60 hover:text-rose transition-colors disabled:opacity-50"
                  >
                    {partyPending ? (
                      <Loader2 className="w-3.5 h-3.5 animate-spin" aria-hidden />
                    ) : (
